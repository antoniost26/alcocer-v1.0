...
module.exports = {
    callback: async (client, interaction) => {
        ...
        if (targetUser.roles.cache.some(role => role.id === staffRole.id)) {
            await interaction.editReply(`${userMention(targetUser.user.id)} is already part of the staff.`)
            return;
        }
        

        targetUser.roles.add([staffRole, trialRole]).then(async () => {
            await interaction.editReply(`Successfully updated roles for ${userMention(targetUser.user.id)}. Welcome to our staff team!`)
        }).catch(async (err) => {
            await interaction.editReply(`Encountered the following error when giving roles: ${err}`)
        });
    },
    name: 'addstaff',
    description: 'Hands roles to new staff, use with caution (gives staff + trial mod)',
    // devOnly: Boolean,
    testOnly: true,
    options: [
        {
